...
for i in directories:
    if i not in exclude_folder:
        ...
        readme_file.write('## %s (%s)\n`Picture by` %s<br>`Category` %s\n`Location over` [Google Maps](http://maps.google.com/maps?q=%s,%s) \n\n' %(str(df1['Name'][0]), str(df1['Date'][0]), str(df1['Credit'][0]), str(df1['Category'][0]), str(df1['Latitude'][0]), str(df1['Longitude'][0])))
